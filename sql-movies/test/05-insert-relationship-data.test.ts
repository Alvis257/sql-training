...
const insertMovieGenres = (
  ...
  genreRows.filter(genreRows => genres.includes(genreRows.genre)).map( genres => `(${movieId},${genres.id})`).join(","));
};
...
const insertMovieActors = (
  ...
  actorRows.filter(actorRows => actors.includes(actorRows.full_name)).map( actors => `(${movieId},${actors.id})`).join(","));
};
...
const insertMovieDirectors = (
  ...
  directorRows.filter(directorRows => directors.includes(directorRows.full_name)).map( directors => `(${movieId},${directors.id})`).join(","));
};
...
const insertMovieKeywords = (
  ...
  keywordRows.filter(keywordRows => keywords.includes(keywordRows.keyword)).map(keywords => `(${movieId},${keywords.id})`).join(","));
};
...
const insertMovieProductionCompanies = (
  ...
  productionCompanyRows.filter(productionCompanyRows =>  productionCompanies.includes( productionCompanyRows.company_name)).map(productionCompanies => `(${movieId},${productionCompanies.id})`).join(","));
};
